...
select customer_id, 
	   case when trim(upper(customer_gender))='M' then CONCAT('Mr.',upper(customer_fname),' ',upper(customer_lname))
			when trim(upper(customer_gender))='F' then CONCAT('Ms.',upper(customer_fname),' ',upper(customer_lname))
			else CONCAT(customer_fname,' ',customer_lname)
		...
		case when year(customer_creation_date) < 2005 then 'A'
			 when year(customer_creation_date) between 2005 and 2010 then 'B'	
...
select p.product_id,p.product_desc,p.product_price,p.product_quantity_avail,
p.product_quantity_avail*p.product_price as inv_value,
case
when p.product_price > 20000.00 then p.product_price*0.80
when p.product_price > 10000.00 then p.product_price*0.85
when p.product_price <= 10000.00 then p.product_price *0.90
end as new_price
from product p
where not exists (
		select product_id
...
select pc.product_class_code,pc.product_class_desc,count(p.product_id) as product_id_count,
sum(p.product_quantity_avail * p.product_price) AS inv_value
from product p
join  product_class pc on pc.product_class_code = p.product_class_code 
group by pc.product_class_code,pc.product_class_desc
having inv_value > 100000
order by inv_value desc;
...
select oh.customer_id,concat(customer_fname,' ',customer_lname) as cus_full_name,oc.customer_email,
oc.customer_phone as phone,a.country,
(select count(*)
from order_header oh
where oh.customer_id=oc.customer_id and oh.order_status!='cancelled'
) as orders_count
...
SELECT OH.SHIPPER_ID,S.SHIPPER_NAME,A.CITY AS CITY_TO_CATERING,COUNT(DISTINCT OH.CUSTOMER_ID)AS NO_OF_CUSTOMERS,
COUNT(OH.ORDER_ID) AS NO_OF_CONSIGNMENT_DELIVERED
FROM ORDER_HEADER OH
join SHIPPER S  on S.shipper_id = OH.shipper_id AND S.SHIPPER_ID = 50001
...
select pc.product_class_code,p.product_id,pc.product_class_desc,p.product_quantity_avail as inv_qty_stock,
p.product_quantity_avail * product_price as inv_value,sum(oi.product_quantity) as quantity,
case when p.PRODUCT_QUANTITY_AVAIL = 0 THEN 'No Sales in past, give discount to reduce inventory'
	 when pc.PRODUCT_CLASS_DESC ='Electronics' or 'Computer' then
         case when p.PRODUCT_QUANTITY_AVAIL < SUM(OI.PRODUCT_QUANTITY*0.1) then 'Low inventory, need to add inventory'
              when p.PRODUCT_QUANTITY_AVAIL < SUM(OI.PRODUCT_QUANTITY*0.5) then 'Medium inventory, need to add some inventory'
			  when p.PRODUCT_QUANTITY_AVAIL >= SUM(OI.PRODUCT_QUANTITY*0.5) then 'Sufficient inventory'
         end 
	  when pc.PRODUCT_CLASS_DESC = 'MOBILES' or 'WATCHES' then
         case when p.PRODUCT_QUANTITY_AVAIL < SUM(OI.PRODUCT_QUANTITY*0.2) then 'Low inventory, need to add inventory'
              when p.PRODUCT_QUANTITY_AVAIL < SUM(OI.PRODUCT_QUANTITY*0.6) then 'Medium inventory, need to add some inventory'
			  when p.PRODUCT_QUANTITY_AVAIL >= SUM(OI.PRODUCT_QUANTITY*0.6) then 'Sufficient inventory'
		  end
	   when pc.PRODUCT_CLASS_DESC not in( 'MOBILES','WATCHES', 'Electronics','Computer') THEN
          case when p.PRODUCT_QUANTITY_AVAIL < SUM(OI.PRODUCT_QUANTITY*0.3) then 'Low inventory, need to add inventory'
			   when p.PRODUCT_QUANTITY_AVAIL < SUM(OI.PRODUCT_QUANTITY*0.7) then 'Medium inventory, need to add some inventory'
               when p.PRODUCT_QUANTITY_AVAIL >= SUM(OI.PRODUCT_QUANTITY*0.7) then 'Sufficient inventory'
          end
END AS INV_STATUS    
from product p
join product_class pc on pc.product_class_code = p.product_class_code
...
with 
carton_res as (select carton_id, (len*width*height) as cart_volume from carton where carton_id in (10))

select * from 
	(select oi.order_id,SUM(p.len*p.width*p.height*OI.PRODUCT_QUANTITY) as TOT_product_vol
	from product p
	join order_items oi on oi.product_id = p.product_id 
	group by oi.order_id
)tbl
where TOT_product_vol < (select cart_volume from carton_res)
ORDER BY TOT_PRODUCT_VOL DESC
LIMIT 1;
...
SELECT OC.CUSTOMER_ID,CONCAT(UPPER(OC.CUSTOMER_FNAME),' ',UPPER(OC.CUSTOMER_LNAME))AS CUS_FULL_NAME,
SUM(OI.PRODUCT_QUANTITY) as TOTAL_QTY, 
SUM(OI.PRODUCT_QUANTITY*P.PRODUCT_PRICE) as TOTAL_VALUE ,oh.payment_mode
FROM ONLINE_CUSTOMER OC  
join Order_HEADER OH on OH.CUSTOMER_ID = OC.CUSTOMER_ID  and oh.PAYMENT_MODE = 'Cash'
...
with 
dist_order_id as (select distinct order_id from order_items where 1=1 and product_id in (201))

select ot.product_id, p.product_desc, sum(ot.product_quantity) as prod_quantity
from 
(select order_id, product_id, product_quantity 
	from order_items 
	where order_id in (select * from dist_order_id )
	)ot
join product 		 p 	on	p.product_id  = ot.product_id	
join order_header    oh on ot.order_id    = oh.order_id
join online_customer oc on oc.customer_id = oh.customer_id
join address 		 ad on oc.address_id  = ad.address_id
where lower(ad.city) not in ('bangalore','new delhi') and ot.product_id not in (201)
group by 1,2;
...
select oi.order_id,oc.customer_id,
CONCAT(UPPER(OC.CUSTOMER_FNAME),' ',UPPER(OC.CUSTOMER_LNAME))AS CUS_FULL_NAME,
sum(oi.product_quantity) as TOT_QTY, a.pincode 
from online_customer oc
join order_header oh on oh.customer_id = oc.customer_id and oh.order_status='shipped'
join order_items oi on oi.order_id = oh.order_id and mod(oi.order_id,2) = 0
join address a on a.address_id = oc.address_id and a.pincode not like '5%'
group by oi.order_id;
